...
get_prism_normals(type = "ppt", resolution = "800m", mon = 3:7, keepZip = TRUE)
# get_prism_normals(type = "tmean", resolution = "800m", mon = 3:7, keepZip = TRUE)
# get_prism_normals(type = "tmin", resolution = "800m", mon = 3:7, keepZip = TRUE)
# get_prism_normals(type = "tmax", resolution = "800m", mon = 3:7, keepZip = TRUE)
...
precip <- stack(precip_rasters)
names(precip) <- paste0("precip_", 3:7)

# tmean_rasters <- purrr::map(tmean_files, raster)
# tmean <- stack(tmean_rasters)
# names(tmean) <- paste0("tmean_", 3:7)
# 
# tmin_rasters <- purrr::map(tmin_files, raster)
# tmin <- stack(tmin_rasters)
# names(tmin) <- paste0("tmin_", 3:7)
# 
# tmax_rasters <- purrr::map(tmax_files, raster)
# tmax <- stack(tmax_rasters)
# names(tmax) <- paste0("tmax_", 3:7)
...
# climate_rasters <- crop(climate_rasters, extent(points) + 0.2)
climate_rasters <- crop(precip, extent(points) + 0.2)

mean_precip <- mean(climate_rasters)
plot(mean_precip)

distinct_locations$mean_precip <- extract(mean_precip, points)
```
...
observed_mtx <- get_var_wide(cropped_checklists_shuffled, "observed", 5)
distance_mtx <- get_var_wide(cropped_checklists_shuffled, "EFFORT.DISTANCE.KM", 5)
tod_mtx <- get_var_wide(cropped_checklists_shuffled, "tod", 5)
doy_mtx <- get_var_wide(cropped_checklists_shuffled, "yday", 5)
protocol_mtx <- get_var_wide(cropped_checklists_shuffled, "PROTOCOL.CODE", 5)
duration_mtx <- get_var_wide(cropped_checklists_shuffled, "DURATION.MINUTES", 5)
```
...
```{r}
site_covariates <- data.frame(precip = distinct_locations$mean_precip,
...
```{r}
occufit <- occu(~ distance + tod + doy + duration + protocol + precip ~ precip + latitude * longitude, data = occu_frame)
...
# TODO: enforce a specific degree of lat/lon rounding
